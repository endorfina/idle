...
unsigned shift_appendages(std::array<float, Size>& ray_array, Rand& rando)
{
    ...
    return random_int{APPLICATION_FPS * 3, APPLICATION_FPS * 11}(rando);
}
...
bool landing_room::step(graphics::core& gl)
{
    ...
    thing.rotation += .01f / APPLICATION_FPS;
    ...
    {
        thing.legs[1][i] += (thing.legs[0][i] - thing.legs[1][i]) / APPLICATION_FPS * .3f;
    }
